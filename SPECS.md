...
| REQ-1.2.3 | The site SHALL load Inter (body), Space Grotesk (headings), and JetBrains Mono (pricing figures / small labels only) via `next/font`, wired into Tailwind and the root layout. | Must Have | Story E1-F2-S3. Done / Pass. |
...
| Fonts | `next/font` (Inter, Space Grotesk, JetBrains Mono) | Self-hosted, zero-layout-shift font loading consistent with the "Spec Sheet / Blueprint" design direction. |
